  ...
  class Column : public Gtk::TreeModel::ColumnRecord {
   public:
    ...
    Gtk::TreeModelColumn<Glib::ustring>
        num;  // Only used when the view is sort by subtitle.
 ...
 public:
  static void create() {
    ...
    if (m_static_instance == NULL)
      m_static_instance =
          gtkmm_utility::get_widget_derived<DialogErrorChecking>(
              SE_DEV_VALUE(SE_PLUGIN_PATH_UI, SE_PLUGIN_PATH_DEV),
              "dialog-error-checking.ui", "dialog-error-checking");
  ...
  void create_menubar(const Glib::RefPtr<Gtk::Builder> &builder) {
    ...
    m_action_group->add(Gtk::Action::create("Refresh", Gtk::Stock::REFRESH),
                        Gtk::AccelKey("F5"),
                        sigc::mem_fun(*this, &DialogErrorChecking::refresh));
    m_action_group->add(
        Gtk::Action::create("TryToFixAll", Gtk::Stock::APPLY,
                            _("Try To _Fix All")),
        Gtk::AccelKey("F3"),
    ...
    m_action_group->add(
        Gtk::RadioAction::create(sort_group, "SortByCategories",
                                 _("By _Categories")),
        Gtk::AccelKey("<Control>C"),
        sigc::bind(sigc::mem_fun(*this, &DialogErrorChecking::set_sort_type),
                   BY_CATEGORIES));
    m_action_group->add(
        Gtk::RadioAction::create(sort_group, "SortBySubtitles",
                                 _("By _Subtitles")),
    ...
    m_action_group->add(
        Gtk::Action::create("CollapseAll", _("_Collapse All")),
        Gtk::AccelKey("C"),
        sigc::mem_fun(*this, &DialogErrorChecking::on_collapse_all));
    m_action_group->add(
        Gtk::Action::create("ExpandAll", _("_Expand All")), Gtk::AccelKey("E"),
  ...
  void on_selection_changed() {
    ...
    unsigned int num =
        utility::string_to_int(Glib::ustring((*it)[m_column.num]));
  ...
  void set_statusbar_error(unsigned int count) {
    if (count == 0)
      ...
      m_statusbar->push(build_message(
          ngettext("1 error was found.", "%d errors were found.", count),
          count));
  }
  ...
  void add_error(Gtk::TreeModel::Row &node, ErrorChecking::Info &info,
    ...
    if (get_sort_type() == BY_CATEGORIES) {
      Glib::ustring subtitle =
          build_message(_("Subtitle n°<b>%d</b>"), info.currentSub.get_num());
  ...
  void update_node_label(const Gtk::TreeModel::Row row) {
    ...
    if (get_sort_type() == BY_CATEGORIES) {
      ...
      row[m_column.text] = build_message(
          ngettext("%s (<b>1 error</b>)", "%s (<b>%d errors</b>)", size),
          label.c_str(), size);
    } else if (get_sort_type() == BY_SUBTITLES) {
      unsigned int num =
          utility::string_to_int(Glib::ustring(row[m_column.num]));
      row[m_column.text] = build_message(
          ngettext("Subtitle n°<b>%d</b> (<b>1 error</b>)",
                   "Subtitle n°<b>%d</b> (<b>%d errors</b>)", size),
          num, size);
    }
  }
