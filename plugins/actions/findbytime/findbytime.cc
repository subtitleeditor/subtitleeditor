...
class FindByTimePlugin : public Action {
 public:
  ...
  void activate() {
    ...
    action_group->add(
        Gtk::Action::create(
            "find-by-time", _("Find Subtitle By Time"),
    ...
    ui->add_ui(ui_id, "/menubar/menu-tools/find-by-time", "find-by-time",
               "find-by-time");
  }
  ...
  void on_find_by_time() {
    ...
    for (Subtitle cursub = subs.get_first(); cursub;
      ...
      if ((cursub.get_start().totalmsecs <= playerpos) &&
        ...
        distance = std::min(std::abs(playerpos - cursub.get_start().totalmsecs),
                            std::abs(playerpos - cursub.get_end().totalmsecs));
