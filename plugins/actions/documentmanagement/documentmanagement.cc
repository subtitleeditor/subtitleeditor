...
class DocumentManagementPlugin : public Action {
   public:
    ...
    void activate() {
        ...
        action_group->add(
            Gtk::Action::create("save-as-document", Gtk::Stock::SAVE_AS, "", _("Save the current file with a different name or format")),
        ...
        action_group->add(
            Gtk::Action::create("open-translation",
                                Gtk::Stock::OPEN,
                                _("Open _Translation"),
                                _("Open a translation from a file (this will populate and overwrite the Translation column, taking only text from "
                                  "the opened file, which does not have to have the same timecodes or number of subtitles as your document)")),
    ...
    void on_save_translation() {
        ...
        if (doc_translation.save(uri) == false)
            ...
            current->flash_message(
                _("Saving translation file %s (%s, %s, %s)."), filename.c_str(), format.c_str(), encoding.c_str(), newline.c_str());
    }
