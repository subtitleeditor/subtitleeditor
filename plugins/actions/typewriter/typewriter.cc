...
class TypewriterPlugin : public Action {
    ...
    void activate() {
        ...
        action_group->add(
            Gtk::Action::create("typewriter",
                                _("_Typewriter"),
                                _("Produces a typewriter effect by creating several head-to-head subtitles instead of the selected ones in the same "
                                  "total duration making it appear as if letters or words are being added gradually")));
   ...
   protected:
    ...
    std::vector<Subtitle>
    create_subtitles_from_text_array(Subtitles &subtitles, Subtitle &original_subtitle, const std::vector<Glib::ustring> &vtext) {
        // Array for new subtitles
        std::vector<Subtitle> newsubs;
