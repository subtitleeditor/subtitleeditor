   ...
   public:
    ...
    static void create() {
        std::unique_ptr<DialogTimingFromPlayerPreferences> dialog(
            gtkmm_utility::get_widget_derived<DialogTimingFromPlayerPreferences>(SE_DEV_VALUE(SE_PLUGIN_PATH_UI, SE_PLUGIN_PATH_DEV),
...
class TimingFromPlayer : public Action {
   public:
    ...
    void activate() {
        ...
        // preferences
        action_group->add(
            Gtk::Action::create("timing-from-player/preferences", Gtk::Stock::PREFERENCES, _("Preferences"), _("Configure this submenu")),
