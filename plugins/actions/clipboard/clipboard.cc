...
class ClipboardPlugin : public Action {
   public:
    ...
    void activate() {
        ...
        action_group->add(Gtk::Action::create("menu-edit/menu-copy", _("Copy"), _("Copy")));
        ...
        action_group->add(
            Gtk::Action::create("clipboard-copy",
                                _("_Copy"),
        ...
        action_group->add(Gtk::Action::create("clipboard-copy-with-timing",
                                              _("Copy With Timing"),
                                              _("Copy selected subtitles and make their timing "
                                                "visible to other clipboard-using applications")),
                          Gtk::AccelKey("<Control>c"),
                          sigc::mem_fun(*this, &ClipboardPlugin::on_copy_with_timing));

        action_group->add(
            Gtk::Action::create("clipboard-cut", Gtk::Stock::CUT, _("C_ut"), _("Copy selected subtitles to the clipboard and delete them")),
            Gtk::AccelKey("<Control>x"),
            sigc::mem_fun(*this, &ClipboardPlugin::on_cut));

        action_group->add(Gtk::Action::create("menu-edit/menu-paste", _("Paste"), _("Paste")));
        action_group->get_action("menu-edit/menu-paste")->set_stock_id(Gtk::Stock::PASTE);
        action_group->add(Gtk::Action::create("clipboard-paste",
                                              Gtk::Stock::PASTE,
                                              _("_Paste"),
                                              _("Paste subtitles from the clipboard AFTER the "
                                                "currently selected subtitle, keeping their duration and gaps between them but setting the start of "
                                                "the first one right after with respect to the minimal gap between subtitles setting )")),
                          Gtk::AccelKey("<Control>v"),
                          sigc::mem_fun(*this, &ClipboardPlugin::on_paste));

        action_group->add(Gtk::Action::create("clipboard-paste-at-player-position",
                                              _("Paste At Current Player Position"),
                                              _("Paste subtitles from the clipboard AFTER the "
                                                "currently selected subtitle, keeping their duration and gaps between them but setting the start of "
                                                "the first at the current player position")),
                          Gtk::AccelKey("<Control><Shift>v"),
                          sigc::mem_fun(*this, &ClipboardPlugin::on_paste_at_player_position));
        action_group->add(Gtk::Action::create("clipboard-paste-as-new-document",
        ...
        action_group->add(Gtk::Action::create("clipboard-paste-over-text",
                                              _("Paste Over Text"),
                                              _("Overwrite text of the first selected subtitle with clipboard content (if your clipboard holds "
                                                "copied subtitles and more subtitles are selected, their texts will get overwritten one by one)")),
                          sigc::mem_fun(*this, &ClipboardPlugin::on_paste_over_text));
        action_group->add(Gtk::Action::create("clipboard-paste-over-time",
                                              _("Paste Over Time"),
                                              _("Overwrite time of the first selected subtitle with clipboard content (if your clipboard holds "
                                                "copied subtitles and more subtitles are selected, their times will get overwritten one by one)")),
                          sigc::mem_fun(*this, &ClipboardPlugin::on_paste_over_time));
        action_group->add(Gtk::Action::create("clipboard-paste-unchanged",
                                              _("Paste Unchanged"),
        ...
        action_group->add(
            Gtk::Action::create(
                "clipboard-select-overlap", _("Select Clipboard Overlap"), _("Select subtitles that overlap with subtitles in the clipboard")),
      ...
      <ui>
        ...
        connection_owner_change =
            Gtk::Clipboard::get()->signal_owner_change().connect(sigc::mem_fun(*this, &ClipboardPlugin::on_clipboard_owner_change));
        ...
        connection_document_changed =
            se::documents::signal_active_changed().connect(sigc::mem_fun(*this, &ClipboardPlugin::on_active_document_changed));

        connection_player_message =
            get_subtitleeditor_window()->get_player()->signal_message().connect(sigc::mem_fun(*this, &ClipboardPlugin::on_player_message));
   ...
   protected:
    ...
    void update_paste_visibility() {
        ...
        action_group->get_action("clipboard-paste-over-text")->set_sensitive(paste_over_visible);
        action_group->get_action("clipboard-paste-over-time")->set_sensitive(paste_over_visible);
        action_group->get_action("clipboard-paste-unchanged")->set_sensitive(paste_visible);
        action_group->get_action("clipboard-select-overlap")->set_sensitive(select_overlap_visible);
    }
    ...
    void on_active_document_changed(Document *doc) {
        ...
        if (doc) {
            connection_selection_changed =
                doc->get_signal("subtitle-selection-changed").connect(sigc::mem_fun(*this, &ClipboardPlugin::on_selection_changed));
    ...
    void on_clipboard_received_targets(const Glib::StringArrayHandle &targets_array) {
        ...
        for (const auto &target : my_targets) {
            if (std::find(avail_targets.begin(), avail_targets.end(), target.get_target().c_str()) != avail_targets.end()) {
        ...
        se_dbg_msg(SE_DBG_PLUGINS, "The winning target is: '%s'.", chosen_clipboard_target.c_str());
    }
    ...
    void on_clipboard_get(Gtk::SelectionData &selection_data, guint /*info*/) {
        ...
        try {
            se_dbg_msg(SE_DBG_PLUGINS, "Supplying clipboard data as '%s' format.", format.c_str());
            ...
            se_dbg_msg(SE_DBG_PLUGINS, "Failed to save clipboard subtitles as '%s'.", format.c_str());
        }
    }
    ...
    void grab_system_clipboard() {
        ...
        // Targets:
        refClipboard->set(
            my_targets, sigc::mem_fun(*this, &ClipboardPlugin::on_clipboard_get), sigc::mem_fun(*this, &ClipboardPlugin::on_clipboard_clear));
    }
    ...
    void request_clipboard_data() {
        ...
        refClipboard->request_contents(chosen_clipboard_target, sigc::mem_fun(*this, &ClipboardPlugin::on_clipboard_received));
    }
    ...
    void on_clipboard_received(const Gtk::SelectionData &selection_data) {
        ...
        if ((target == target_default) || (target == target_text)) {
            ...
            try {
                ...
                if (target == target_text) {
                    se_dbg_msg(SE_DBG_PLUGINS, "Read the content of clipboard as Plain Text Format");
                    ...
                    se_dbg_msg(SE_DBG_PLUGINS, "Failed to recognize the default target as a subtitle format!");
                    return;
                }
            }
        } else {  //( target == target_default )||( target == target_text )
            se_dbg_msg(SE_DBG_PLUGINS, "Somebody is sending us data as this strange target: '%s'.", target.c_str());
            g_warning(
        ...
        if (need_generate_extension) {
            ...
            Glib::ustring ext = SubtitleFormatSystem::instance().get_extension_of_format(fmt);
    ...
    bool is_something_to_paste() {
        if (clipdoc == NULL) {
            se_dbg_msg(SE_DBG_PLUGINS, "No clipboard document, nothing to paste. How come I was called?");
            return false;
        } else if (clipdoc->subtitles().size() == 0) {
            se_dbg_msg(SE_DBG_PLUGINS, "No subtitles in the clipboard document - how come I was called?");
            return false;
        }
        return true;
    }
    ...
    void calculate_and_apply_timeshift(Subtitles &subtitles, Subtitle &paste_after, std::vector<Subtitle> &new_subtitles, unsigned long flags) {
        ...
        for (auto &sub : new_subtitles) {
            sub.set_start_and_end(sub.get_start() + timeshift, sub.get_end() + timeshift);
        }
    }
    ...
    void set_pastedoc(Document *doc) {
        ...
        connection_pastedoc_deleted = se::documents::signal_deleted().connect(sigc::mem_fun(*this, &ClipboardPlugin::on_pastedoc_deleted));
    }
