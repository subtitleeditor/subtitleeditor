   ...
   if (action == "new-style") {
      ...
      if (m_current_document->styles().size() > 0 && (!script_info.data.count("PlayResX") || !script_info.data.count("PlayResX"))) {
         ...
         dialog.set_secondary_text(
            ...
            "style "
            "will be added then) and fix the situation manually.\n\nOr apply a default screen size (1920x1080). A new style will be added then, "
            "but it "
            "might make subtitles in the current styles look too big or too small");
