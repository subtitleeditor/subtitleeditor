...
DialogStyleEditor::DialogStyleEditor(BaseObjectType *cobject, const Glib::RefPtr<Gtk::Builder> &builder) : Gtk::Dialog(cobject) {
...
#define init_widget(WidgetClass, widget_name, signal, callback, key)                                                \
    {                                                                                                               \
        ...
        w->signal().connect(sigc::bind(sigc::mem_fun(*this, &DialogStyleEditor::callback), w, Glib::ustring(key))); \
    }
    ...
    init_widget(Gtk::Button, "button-set-default", signal_clicked, callback_button_clicked, "set-default");

    init_widget(Gtk::SpinButton, "spin-playres-x", signal_value_changed, callback_playres_changed, "playres-x");
    init_widget(Gtk::SpinButton, "spin-playres-y", signal_value_changed, callback_playres_changed, "playres-y");
    ...
    init_widget(Gtk::RadioButton, "radio-box-per-line", signal_toggled, callback_radio_toggled, "box-per-line");
    init_widget(Gtk::RadioButton, "radio-rectangular-box", signal_toggled, callback_radio_toggled, "rectangular-box");
    ...
    for (unsigned int i = 0; i < 9; ++i) {
        ...
        w->signal_toggled().connect(sigc::bind(sigc::mem_fun(*this, &DialogStyleEditor::callback_alignment_changed), w, i + 1));
    }
    ...
    // create treeview (this is in braces to destroy the pointers after we do not need them)
    {
        ...
        renderer->signal_edited().connect(sigc::mem_fun(*this, &DialogStyleEditor::on_style_name_edited));
        ...
        m_treeview->get_selection()->signal_changed().connect(sigc::mem_fun(*this, &DialogStyleEditor::callback_style_selection_changed));
...
void DialogStyleEditor::callback_button_clicked(Gtk::Button *, const Glib::ustring &action) {
    ...
    if (action == "new-style") {
        ...
        if (m_current_document->styles().size() > 0 && (!script_info.data.count("PlayResX") || !script_info.data.count("PlayResX"))) {
            ...
            Gtk::MessageDialog dialog(*this, _("PlayResX or PlayResY not set"), false, Gtk::MESSAGE_INFO, Gtk::BUTTONS_CANCEL);
            dialog.set_title(_("Missing PlayRes Information"));
            dialog.set_secondary_text(
                "You tried to add a new style, but this document does not have target screen size set and already has styles.\n\n Either cancel (no "
                "style will be added then) and fix the situation manually.\n\nOr apply a default screen size (1920x1080). A new style will be added "
                "then, but it might make subtitles in the current styles look too big or too small");
            ...
            if (result != RESPONSE_APPLY_DEFAULT)
                ...
                dynamic_cast<Gtk::SpinButton *>(m_widgets["spin-playres-x"])->set_sensitive(true);
                dynamic_cast<Gtk::SpinButton *>(m_widgets["spin-playres-y"])->set_sensitive(true);
            }
        }
        ...
        if (m_current_style) {
            ...
            cfg::set_comment("AdvancedSubStationAlpha", "default-style", "Default style to be used");
...
void DialogStyleEditor::callback_playres_changed(Gtk::SpinButton *w, const Glib::ustring &key) {
...
void DialogStyleEditor::init_style(const Style &style) {
    ...
    if (m_current_document) {
        ...
        if (script_info.data.count("PlayResX")) {
            ...
            dynamic_cast<Gtk::SpinButton *>(m_widgets["spin-playres-x"])->set_value(playres_x);
        } else {
            dynamic_cast<Gtk::SpinButton *>(m_widgets["spin-playres-x"])->set_sensitive(false);
        }

        if (script_info.data.count("PlayResY")) {
            playres_y = utility::string_to_int(script_info.data["PlayResY"]);
            dynamic_cast<Gtk::SpinButton *>(m_widgets["spin-playres-y"])->set_value(playres_y);
        } else {
            dynamic_cast<Gtk::SpinButton *>(m_widgets["spin-playres-y"])->set_sensitive(false);
        }
    }
...
#define init_color_button(name, key)                                               \
    ...
    // border style
    {
        ...
        if (border_style == "1")
            ...
            dynamic_cast<Gtk::RadioButton *>(m_widgets["radio-rectangular-box"])->set_active(true);
        else
            dynamic_cast<Gtk::RadioButton *>(m_widgets["radio-box-per-line"])->set_active(true);
    }
    // alignment
    {
...
class StyleEditorPlugin : public Action {
   public:
    ...
    void activate() {
        ...
        action_group->add(Gtk::Action::create("style-editor", Gtk::Stock::SELECT_COLOR, _("_Style Editor"), _("Launch the style editor")),
