...
class BestFitPlugin : public Action {
  ...
  void activate() {
    ...
    action_group->add(
        Gtk::Action::create(
            ...
            _("Equalize the speed (in CPS) of the selected subtitles between "
              "the start of the first and the end of the last one (while "
              "respecting the minimum gap between subtitles and their minimal "
              "duration)")),
 ...
 protected:
  ...
  void bestfit(std::vector<Subtitle> &subtitles) {
    ...
    bool done =
        false;  // we're done if no subtitle duration hit the minimum limit
    bool hopeless =
        false;  // if all durations hit the minimum limit, it's hopeless
    while (!done && !hopeless) {
      done = true;
      hopeless = true;
