...
Document::Document(bool create_new) : CommandSystem(*this), m_subtitles(*this), m_styles(*this) {
    ...
    if (create_new && (m_format == "Advanced Sub Station Alpha" || m_format == "Sub Station Alpha" || m_format == "Subtitle Editor Project")) {
    ...
    CommandSystem::signal_changed().connect(sigc::mem_fun(*this, &Document::make_document_changed));
}

// Constructor by copy
Document::Document(Document &src, bool copy_subtitles) : CommandSystem(*this), m_subtitles(*this), m_styles(*this) {
...
bool Document::save(const Glib::ustring &uri) {
    ...
    try {
        ...
        Glib::ustring title = build_message(
            _("Could not save the file \"%s\" using the character coding %s."), basename.c_str(), Encodings::get_label_from_charset(charset).c_str());
        Glib::ustring msg =
            _("The document contains one or more characters "
...
Document *Document::create_from_file(const Glib::ustring &uri, const Glib::ustring &charset) {
    ...
    try {
        ...
        Glib::ustring title = build_message(_("Could not recognize the subtitle format for the file \"%s\"."), basename.c_str());
        Glib::ustring msg =
        ...
        if (charset.empty()) {
            ...
            msg =
                _("Subtitle Editor was not able to automatically determine the file "
                  "encoding. "
                  "Select a different character coding from the menu and try again.");
        } else {
            title = build_message(_("Could not open the file \"%s\" using the character coding %s."),
                                  basename.c_str(),
                                  Encodings::get_label_from_charset(charset).c_str());
